...
go run ./cmd/ops-scheduler --env prod --env-file configs/environments.yaml --policy configs/policies.yaml --audit audit/scheduler.jsonl --discover-interval 6h --discover-apply --discover-allow-ports 80,443,8080 --discover-allow-names 'api*,nginx*' --discover-max-additions 5
```
...
If `--discover-interval` is set, the scheduler also runs the SSH-based discovery flow on that lower cadence before health checks. By default this is report-only; it will not mutate `configs/environments.yaml` unless you also set `--discover-apply`.
When `--discover-apply` is enabled, use `--discover-allow-ports`, `--discover-allow-names`, and `--discover-max-additions` to keep noisy hosts from auto-enrolling too many services at once.
