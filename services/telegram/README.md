...
- Surfaces incident `highlights` first in `/health` replies so runtime signals like `CONTAINER_OOMKILLED`, triggered Prometheus metric signals, or recent systemd errors are visible before the longer result list, and also shows recent change markers plus suggestion `strategy` values such as `restart_candidate`, `change_regression`, or `capacity`
